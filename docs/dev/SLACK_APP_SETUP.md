...
process, one Slack app, one token** — see `server/README.md` for why (Slack
ties one Interactivity Request URL to one app; two separately-deployed
services can't both be it).
...
> **`<bot-host>`** is the dev toolkit EC2 instance's public URL. The
> `infrastructure/terraform/stacks/toolkit` stack (module `toolkit-ec2`)
> deploys it on every merge to `dev`, and CD health-checks `/health` before
> declaring success. Read the live value with
> `tofu output` from `infrastructure/terraform/stacks/toolkit`, or use
> `ngrok` against a local `uv run uvicorn main:app` for testing.
...
`ATTIO_WEBHOOK_SECRET` (webhook signing) is also required for `ddl_commands`.
`/find-match` needs none of this.

## Before enabling for real users

1. **Confirm the bot is actually deployed and reachable** — the dev toolkit
   host is redeployed and `/health`-checked on every merge to `dev`; confirm
   the latest deploy succeeded and the Interactivity URL points at its
   current public URL.
