...
| P7 | Delete the DEV-workspace webhook. Announce the local `.env` change (every developer's `ATTIO_API_KEY` and `ATTIO_NOTE_OBJECT_SLUG`; `ATTIO_DEAL_OBJECT_SLUG` is deleted). Clear the persisted user-scoped `DEV_ATTIO_API_KEY`. | `.env.example` is updated in the repo but existing `.env` files are untracked. **A blank `ATTIO_NOTE_OBJECT_SLUG=` now fails settings construction outright**, so anyone whose `.env` still has the old empty value gets a hard startup error naming the field — set it to `note`. That is deliberate: the alternative was note pushes silently doing nothing. A stale user-scoped `DEV_ATTIO_API_KEY` invites a muscle-memory run against a dead workspace. |
