...
COMMANDS_JSON=$(jq -n \
  --arg image_cmd 'CID=$(docker ps -a --filter "label=com.docker.compose.service=toolkit" --format "{{.ID}}" | head -1); test -n "$CID" || { echo "toolkit compose service container does not exist" >&2; exit 1; }; IMAGE=$(docker inspect --format="{{.Image}}" "$CID"); test -n "$IMAGE" || { echo "could not resolve toolkit image from container $CID" >&2; exit 1; }' \
