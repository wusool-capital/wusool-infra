...
impl MeetingsRepository {
    pub async fn get_meetings(pool: &SqlitePool) -> Result<Vec<MeetingModel>, sqlx::Error> {
        // LEFT JOIN + MAX(audio_end_time) computes each meeting's total
        // duration from its transcript segments on the fly (no stored
        // column to keep in sync) -- meetings with no transcripts yet
        // (still recording, or empty) get duration_seconds = NULL.
        let meetings = sqlx::query_as::<_, MeetingModel>(
            "SELECT m.*, MAX(t.audio_end_time) as duration_seconds \
    ...
    pub async fn get_meeting(
        ...
        let meeting: Option<MeetingModel> =
            sqlx::query_as("SELECT id, title, created_at, updated_at, folder_path, push_tag, pushed_at FROM meetings WHERE id = ?")
                .bind(meeting_id)
    ...
    /// Loads every local meeting's identity + sync-relevant fields in one
    /// query, for sync_pushed_meetings (push/mod.rs) to match against
    /// the server's install_id-scoped list without a get_meeting_metadata
    /// round trip per remote item.
    pub async fn list_all_metadata(pool: &SqlitePool) -> Result<Vec<MeetingModel>, SqlxError> {
        let meetings: Vec<MeetingModel> = sqlx::query_as(
            "SELECT id, title, created_at, updated_at, folder_path, push_tag, pushed_at FROM meetings",
        )
        .fetch_all(pool)
        .await?;
        Ok(meetings)
    ...
    /// Sets the buyer/seller grouping tag for a meeting (push_tag column
    /// -- distinct from folder_path, which is the local filesystem
    /// folder holding the recording's audio files).
    pub async fn update_meeting_push_tag(
        pool: &SqlitePool,
        meeting_id: &str,
        push_tag: &str,
    ) -> Result<bool, SqlxError> {
        ...
        let now = Utc::now();
        let rows_affected =
            sqlx::query("UPDATE meetings SET push_tag = ?, updated_at = ? WHERE id = ?")
                .bind(push_tag)
                .bind(now)
                .bind(meeting_id)
                .execute(pool)
    ...
    /// Marks a meeting as pushed (called by push_meeting on success).
    /// Persisted so editing stays locked across app restarts, not just
    /// for the current session's React state.
    pub async fn mark_meeting_pushed(
        pool: &SqlitePool,
        meeting_id: &str,
    ) -> Result<bool, SqlxError> {
        if meeting_id.trim().is_empty() {
        ...
        let now = Utc::now();
        let rows_affected =
            sqlx::query("UPDATE meetings SET pushed_at = ?, updated_at = ? WHERE id = ?")
                .bind(now)
                .bind(now)
                .bind(meeting_id)
                .execute(pool)
