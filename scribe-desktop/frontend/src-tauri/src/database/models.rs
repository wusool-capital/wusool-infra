...
pub struct MeetingModel {
    ...
    /// Business-facing buyer/seller grouping tag (e.g. "Buyer - Acme
    /// Corp"), set by the user before pushing a meeting to the Scribe
    /// backend. Distinct from folder_path, which is the local
    /// filesystem folder holding the recording's audio files.
    pub push_tag: Option<String>,
    /// Set once push_meeting succeeds; persists the "already pushed"
    /// lock (disables further transcript edits, hides the push option)
    /// across app restarts, not just in-session React state.
    pub pushed_at: Option<String>,
    /// Total recording length in seconds, derived from the max
    /// transcript segment end-time -- not a stored column, computed by
    /// get_meetings()'s aggregate join against transcripts. #[sqlx(default)]
    /// so other queries selecting an explicit column list (get_meeting)
    /// don't need to name it -- they just get None.
    #[sqlx(default)]
