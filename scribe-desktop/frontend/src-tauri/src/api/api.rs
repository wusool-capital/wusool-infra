    ...
    match MeetingsRepository::get_meeting_metadata(pool, &meeting_id).await {
        Ok(Some(meeting)) => {
            ...
            Ok(MeetingMetadata {
                ...
                push_tag: meeting.push_tag,
                pushed_at: meeting.pushed_at,
            })
        }
        Ok(None) => {
            log_warn!("Meeting not found: {}", meeting_id);
...
/// Edits one transcript segment's text. Callers must check the meeting
/// isn't pushed yet (see Meeting.pushed_at) -- this command itself
/// doesn't enforce that lock, matching TranscriptsRepository's contract.
#[tauri::command]
pub async fn update_transcript_text<R: Runtime>(
    _app: AppHandle<R>,
    state: tauri::State<'_, AppState>,
...
pub async fn open_meeting_folder<R: Runtime>(
    ...
    let meeting: Option<MeetingModel> = sqlx::query_as(
        "SELECT id, title, created_at, updated_at, folder_path, push_tag, pushed_at FROM meetings WHERE id = ?",
    )
    .bind(&meeting_id)
    .fetch_optional(pool)
    .await
