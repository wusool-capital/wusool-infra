...
pub fn run() {
    ...
    builder
        ...
        .setup(|_app| {
            ...
            // Meeting-detection: per-process mic attribution shows a "start
            ...
            // Periodic background sync for summaries the "Check" button never
            // reached (see push::sync_pushed_meetings) -- runs in the main
            // process rather than a frontend setInterval because closing the
            // window hides it to tray (see on_window_event below), and a
            // hidden WKWebView's DOM timers get throttled/suspended by the
            // OS, which is exactly the case this exists to cover. The
            // frontend still triggers one sync itself on launch (before this
            // first tick) and on demand via the "Check now" button; this
            // loop is the safety net for an app left open unattended.
            // Delayed 10s so it doesn't contend with whisper/parakeet/model
            // init above, and its own AppState guard makes an unmanaged
            // first-launch state a harmless no-op rather than a panic.
            let sync_app_handle = _app.handle().clone();
            tauri::async_runtime::spawn(async move {
                ...
                loop {
                    interval.tick().await;
                    match push::sync_pushed_meetings(sync_app_handle.clone()).await {
                        Ok(result) if result.synced > 0 => {
                            log::info!(
                                "Background sync filed {} summary/ies ({} checked)",
                                result.synced,
                                result.checked
                            );
                            if let Err(e) = sync_app_handle.emit("pushed-summaries-synced", result.synced) {
                                log::warn!("Failed to emit pushed-summaries-synced: {}", e);
                            }
                        }
                        Ok(_) => {}
