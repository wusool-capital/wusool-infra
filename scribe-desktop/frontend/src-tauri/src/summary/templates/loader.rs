...
/// - macOS: ~/Library/Application Support/WusoolScribe/templates/
/// - Windows: %APPDATA%\WusoolScribe\templates\
/// - Linux: ~/.config/WusoolScribe/templates/
fn get_custom_templates_dir() -> Option<PathBuf> {
    let mut path = dirs::data_dir()?;
    path.push("WusoolScribe");
    path.push("templates");
    Some(path)
}
