...
export function OnboardingProvider({ children }: { children: React.ReactNode }) {
  ...
  const completeOnboarding = async () => {
    try {
      ...
      // Local summarization is disabled (summaries are generated
      // server-side by Scribe after a push), so onboarding no longer
      // downloads or selects a local summary model at all.
      await invoke('complete_onboarding', {
        model: '',
      });
      setCompleted(true);
      console.log('[OnboardingContext] Onboarding completed (no local summary model)');
