  ...
  return (
    <div className={`flex items-center space-x-2 ${className}`}>
      ...
      <div className={`w-2 h-2 rounded-full ${
        isActive ? 'bg-success/70 animate-pulse' : 'bg-muted-foreground/30'
      }`} title={`${deviceName} - ${isActive ? 'Active' : 'Inactive'}`} />

      {/* Level meter container */}
      <div className={`flex-1 ${sizes.container} relative`}>
        {/* Background */}
        <div className="w-full h-full bg-accent rounded-sm overflow-hidden">
...
export function CompactAudioLevelMeter({
  ...
  const getLevelColor = (level: number) => {
    if (level < 0.3) return 'bg-success/70';
    if (level < 0.7) return 'bg-warning/70';
  ...
  return (
    <div className={`flex items-center space-x-1 ${className}`}>
      ...
      <div className={`w-1.5 h-1.5 rounded-full ${
        isActive ? 'bg-success/70' : 'bg-muted-foreground/30'
      }`} />

      {/* Mini meter */}
      <div className="w-8 h-1.5 bg-accent rounded-sm overflow-hidden">
        <div
