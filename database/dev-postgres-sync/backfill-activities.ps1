...
# One-time historical backfill of the (Postgres-only, never written to Attio)
# `activities` table -- see the Wusool Schema Handover artifact's Activities
# section for the full design. Reads SOURCE directly because the boundary
# timestamps this needs (first/last/next_*_interaction, Seller Role's raw
# attempt_1/2/3) only exist on SOURCE -- DEV already collapsed them down to
# single latest-value fields (last_interaction_at, last_attempt_*) during the
# SOURCE->DEV migration, so DEV alone can't reconstruct this history.
#
...
SOURCE_KEY = os.environ["WUSOOL_SOURCE_ATTIO_API_KEY"]
DEV_KEY = os.environ["WUSOOL_DEV_ATTIO_API_KEY"]
BASE = "https://api.attio.com/v2"

def request(method, key, path, body=None):
...
def pages(key, path):
    result, offset = [], 0
    while True:
        batch = request("POST", key, path, {"limit": 500, "offset": offset}).get("data", [])
        result.extend(batch)
...
source_companies = pages(SOURCE_KEY, "/objects/companies/records/query")
source_people = pages(SOURCE_KEY, "/objects/people/records/query")
source_mandates = pages(SOURCE_KEY, "/lists/buy_side_mandates/entries/query")
source_sellers = pages(SOURCE_KEY, "/lists/valuation_tool_leads/entries/query")

print("Reading DEV (to resolve SOURCE -> DEV via legacy_attio_id)...")
dev_orgs = pages(DEV_KEY, "/objects/organizations/records/query")
dev_people = pages(DEV_KEY, "/objects/person/records/query")
...
def add_boundary_rows(source_records, subject_type, crosswalk):
    field_channel = [
    ...
    for r in source_records:
        v = vals(r)
        sid = record_id(r)
        dev_id = crosswalk.get(sid)
        if not dev_id: continue
        for field, channel in field_channel:
            ts = first(v, field)
            if not ts: continue
            rows.append((subject_type, dev_id, None, ts, channel, None, None, f"SOURCE {field} backfill"))

add_boundary_rows(source_companies, "Organization", org_by_legacy)
add_boundary_rows(source_people, "Person", person_by_legacy)

for e in source_mandates:
    v = vals(e)
    ts = first(v, "outreach_started_date")
    if not ts: continue
    sid = parent_id(e)
    dev_id = org_by_legacy.get(sid)
    if not dev_id: continue
    rows.append(("Mandate", dev_id, None, ts, "Outreach", "Out", None, "SOURCE outreach_started_date backfill"))
...
seller_org_ids = set()
seller_rows_pending = []
for e in source_sellers:
    v = vals(e)
    sid = parent_id(e)
    dev_org_id = org_by_legacy.get(sid)
    if not dev_org_id: continue
    for date_f, channel_f, outcome_f, label in attempts:
        ts = first(v, date_f)
        if not ts: continue
        channel = first(v, channel_f) or "Other"
        outcome = first(v, outcome_f)
        seller_org_ids.add(dev_org_id)
        seller_rows_pending.append((dev_org_id, ts, channel, outcome, label))

counts = {
    "Organization": sum(1 for r in rows if r[0] == "Organization"),
    "Person": sum(1 for r in rows if r[0] == "Person"),
    "Mandate": sum(1 for r in rows if r[0] == "Mandate"),
...
with psycopg.connect(os.environ["WUSOOL_DATABASE_URL"], connect_timeout=10) as conn:
  with conn.cursor() as c:
    ...
    # Resolve SellerRole subject_uuid via Postgres seller_roles.id (keyed by org_attio_id)
    ...
    for dev_org_id, ts, channel, outcome, label in seller_rows_pending:
        seller_role_id = seller_role_id_by_org.get(dev_org_id)
    ...
    inserted = 0
    for subject_type, subject_attio_id, subject_uuid, ts, channel, direction, outcome, source in rows:
        c.execute(
            "DELETE FROM activities WHERE subject_type=%s AND source=%s "
            "AND subject_attio_id IS NOT DISTINCT FROM %s "
            "AND subject_uuid IS NOT DISTINCT FROM %s::uuid",
            (subject_type, source, subject_attio_id, subject_uuid),
        )
        c.execute(
            """INSERT INTO activities(subject_type, subject_attio_id, subject_uuid, ts, channel, direction, outcome, source)
               VALUES (%s,%s,%s,%s,%s,%s,%s,%s)""",
            (subject_type, subject_attio_id, subject_uuid, ts, channel, direction, outcome, source),
        )
        inserted += 1
    conn.commit()
