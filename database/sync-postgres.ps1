...
with psycopg.connect(os.environ["WUSOOL_DATABASE_URL"], connect_timeout=10) as conn:
  with conn.cursor() as c:
    ...
    for e in seller_entries:
      ...
      seller_rows.append((org,first(v,"outreach_tier"),first(v,"appetite_signal"),first(v,"relationship_status"),J(money(v,"est_revenue")),J(money(v,"est_ebitda")),J(money(v,"owner_salary")),J(money(v,"valuation_low")),J(money(v,"valuation_mid")),J(money(v,"valuation_high")),first(v,"sell_timeline"),number(v,"readiness_score"),first(v,"readiness_band"),mandate_uuid.get(mandate),first(v,"last_attempt_date"),first(v,"last_attempt_channel"),first(v,"last_attempt_outcome"),number(v,"lead_quality_score"),first(v,"re_engage_date"),boolean(v,"is_active"),entry_id(e),J(e)))
    c.executemany("""INSERT INTO seller_roles(org_attio_id,outreach_tier,appetite_signal,relationship_status,est_revenue,est_ebitda,owner_salary,valuation_low,valuation_mid,valuation_high,sell_timeline,readiness_score,readiness_band,mandate_id,last_attempt_date,last_attempt_channel,last_attempt_outcome,lead_quality_score,re_engage_date,is_active,legacy_entry_id,raw_attio)
      VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)
      ON CONFLICT(org_attio_id) DO UPDATE SET outreach_tier=excluded.outreach_tier,appetite_signal=excluded.appetite_signal,relationship_status=excluded.relationship_status,est_revenue=excluded.est_revenue,est_ebitda=excluded.est_ebitda,owner_salary=excluded.owner_salary,valuation_low=excluded.valuation_low,valuation_mid=excluded.valuation_mid,valuation_high=excluded.valuation_high,sell_timeline=excluded.sell_timeline,readiness_score=excluded.readiness_score,readiness_band=excluded.readiness_band,mandate_id=excluded.mandate_id,last_attempt_date=excluded.last_attempt_date,last_attempt_channel=excluded.last_attempt_channel,last_attempt_outcome=excluded.last_attempt_outcome,lead_quality_score=excluded.lead_quality_score,re_engage_date=excluded.re_engage_date,is_active=excluded.is_active,legacy_entry_id=excluded.legacy_entry_id,raw_attio=excluded.raw_attio,updated_at=now()""",seller_rows)
