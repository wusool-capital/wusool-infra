...
| `008_bot_managed_columns.sql` | Add `archived_at`/`bot_managed_at`/`bot_managed_by` to `buyer_roles`/`seller_roles`, for the ddl-commands Slack bot's write commands (soft-delete + sync-collision guard, see `sync-postgres.ps1`). Additive only. |
