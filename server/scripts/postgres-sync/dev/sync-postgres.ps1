...
with psycopg.connect(os.environ["WUSOOL_DATABASE_URL"], connect_timeout=10) as conn:
  with conn.cursor() as c:
    ...
    # Keyed by legacy_entry_id now (one row per DEV entry, not per org --
    # see BuyerRole's docstring, 2026-08-28): a hard delete by org_attio_id
    # would wipe every entry for an org that still has other entries.
    buyer_entry_ids_list=[r[-2] for r in buyer_rows]
    if not buyer_entry_ids_list: raise RuntimeError("DEV Attio returned zero buyer_role entries; refusing to delete all buyer_roles.")
    ...
    # Keyed by legacy_entry_id now (one row per DEV entry, not per org --
    # see SellerRole's docstring, 2026-08-28): a hard delete by org_attio_id
    # would wipe every entry for an org that still has other entries.
    seller_entry_ids_list=[r[-2] for r in seller_rows]
    if not seller_entry_ids_list: raise RuntimeError("DEV Attio returned zero seller_role entries; refusing to delete all seller_roles.")
