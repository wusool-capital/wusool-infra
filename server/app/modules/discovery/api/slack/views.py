...
result of a lead search as a message — one "Add as seller" button per
lead, `action_id="discover_add_seller"` (registered in `handlers.py`),
carrying the lead itself as the button value so the click needs no
server-side lookup to act on it (see `api/dependencies.encode_lead`).
"""
