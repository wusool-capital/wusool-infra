...
an industry/geography/channel, never anything about how discovery works
internally. Exported via this module's root `__all__`.
"""
...
async def find_and_post_leads(*, industry: str, geography: str, channel_id: str) -> None:
    ...
    try:
        leads = await discovery_service().find_leads(
            industry=industry, geography=geography, limit=settings.discovery_lead_search_limit
        )
    except Exception:
