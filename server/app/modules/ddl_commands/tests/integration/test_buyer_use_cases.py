...
async def test_concurrent_create_yields_one_active_role() -> None:
    """Two `/add-buyer` submissions for the same org, landing inside the
    ...
    try:
        async with sessionmaker() as session:
            session.add(Organization(attio_id=attio_id, name="Concurrent Buyer Co"))
            await session.commit()
    ...
    try:
        ...
        winners = [r for r in results if isinstance(r, BuyerRole)]
        losers = [r for r in results if isinstance(r, BuyerAlreadyExistsError)]
        ...
        async with sessionmaker() as session:
            active = await session.scalar(
                select(func.count())
                .select_from(BuyerRole)
                .where(BuyerRole.org_attio_id == attio_id, BuyerRole.is_active.is_(True))
            )
        assert active == 1
    finally:
        async with sessionmaker() as session:
            await session.execute(delete(BuyerRole).where(BuyerRole.org_attio_id == attio_id))
