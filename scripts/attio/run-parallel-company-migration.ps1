...
$scriptPath = Join-Path $repoRoot "scripts\attio\migrate-companies-fast.ps1"
...
for ($worker = 0; $worker -lt $WorkerCount; $worker++) {
  ...
  Write-Host "Starting company worker $($worker + 1): offset=$workerOffset maxRecords=$workerMaxRecords"

  $jobs += Start-Job -Name "companies-$workerOffset" -ScriptBlock {
    param(
      [string]$RepoRoot,
      [string]$ScriptPath,
      [string]$SourceApiKey,
...
if ($failedJobs.Count -gt 0) {
  throw "$($failedJobs.Count) parallel company migration worker(s) failed."
}

Write-Host "Parallel company migration complete."
