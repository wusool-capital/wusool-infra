param(
 [ValidateSet("organizations","person","deal","buyer_role","seller_role")][string]$Entity,
...
$fields = @(
  ...
  # avatar_url field, which already does this), even on a custom object.
  # logo_url alone does not get that treatment. See objects.ps1's field
  # mapping for where the same SOURCE value gets written to both.
  [pscustomobject]@{ Title = "Avatar URL"; Slug = "avatar_url"; Type = "text"; Multi = $false; Required = $false; Unique = $false; SourceOption = $null },
  ...
  # Confirmed live against DEV (2026-08-27): "lead_source" is unconditionally
  # written by objects.ps1 for every Organization record ("Inbound"/
  # "Outbound") but was missing from this field list entirely -- it exists
...
$fields = @(
  ...
  [pscustomobject]@{ Title = "Organization"; Slug = "company"; Type = "record-reference"; Multi = $false; Required = $false; Unique = $false; SourceOption = $null; AllowedObject = "organizations" },
...
$deal=Request Get "/objects/deal" $null
if([string]$deal.data.api_slug-ne"deal"){throw "The custom deal object was not found. Create it manually first (Title: Deal, Slug: deal)."}
...
$attrs=Map @((Request Get "/objects/deal/attributes" $null).data)
$allAttrs=Map @((Request Get "/objects/deal/attributes?show_archived=true" $null).data)
function Clear-ArchivedSlug($slug,$title){
  ...
  if($Apply){
    Request Patch "/objects/deal/attributes/$archivedId" @{data=@{title="$title (Legacy)";api_slug=$legacySlug;is_archived=$true}}|Out-Null
...
# Fresh custom object -- no existing data, so no RenameFrom entries needed
# (unlike DEV's copy of this script, which renamed legacy fields on the
# already-populated native Deals object). "name" and "value" are added here
# explicitly: a plain custom object doesn't get them for free the way
# Attio's native Deals object does. "value" can be a normal currency field
# we fully control (default USD) -- it's not a System attribute here, unlike
# DEV's native Deals.value which Attio permanently locks.
$fields=@(
  # Renamed 2026-08-27 to match DEV's naming convention (deal_name/
  # deal_stage/deal_owner/deal_value) -- deal_stage already exists under
  # that name (created manually via the UI), these three didn't match it.
  # RenameFrom preserves the 2 already-migrated Mandate deals' data instead
  # of creating a duplicate attribute.
  [pscustomobject]@{Title="Deal Name";Slug="deal_name";Type="text";Config=@{};RenameFrom="name"},
  # Missing from this list entirely until now (2026-08-27) -- objects.ps1's
  # Invoke-Deals writes legacy_attio_id on every Deal record unconditionally
  # for idempotent matching (see $byLegacy there), so it must exist. Caught
  # live: a real Deal apply run 400'd with "Cannot find attribute with
  # slug/ID legacy_attio_id" on the very first record.
  [pscustomobject]@{Title="Legacy Attio ID";Slug="legacy_attio_id";Type="text";Config=@{};RenameFrom=$null},
  # Confirmed live (2026-08-27): a plain custom object gets no "Owner" field
  # for free, unlike DEV's native Deals object -- objects.ps1's Invoke-Deals
  # writes to it unconditionally when an owner resolves, so it must exist.
  [pscustomobject]@{Title="Deal Owner";Slug="deal_owner";Type="actor-reference";Config=@{};RenameFrom="owner"},
  [pscustomobject]@{Title="Deal Value";Slug="deal_value";Type="currency";Config=@{currency=@{default_currency_code="USD";display_type="symbol"}};RenameFrom="value"},
  ...
  [pscustomobject]@{Title="Contract Signed Date";Slug="contract_signed_date";Type="date";Config=@{};RenameFrom=$null},
  [pscustomobject]@{Title="Exclusivity Date";Slug="exclusivity_date";Type="date";Config=@{};RenameFrom=$null},
  ...
  [pscustomobject]@{Title="Estimated Deal Value (USD)";Slug="estimated_deal_value_usd";Type="number";Config=@{};RenameFrom=$null},
...
foreach($field in $fields){
  if($attrs.ContainsKey($field.Slug)){
    ...
    if([string]$current.type-ne$field.Type-or[bool]$current.is_multiselect-ne[bool]$field.Multi){throw "deal/$($field.Slug) has unexpected type or cardinality."}
    if([string]$current.title-ne$field.Title){
      $actions.Add("update_title:$($field.Slug):$($field.Title)")
      if($Apply){
        Request Patch "/objects/deal/attributes/$($current.id.attribute_id)" @{data=@{title=$field.Title}}|Out-Null
    ...
    if($field.Type-eq"currency"-and$field.Config.currency-and
      ...
      if($Apply){
        Request Patch "/objects/deal/attributes/$($current.id.attribute_id)" @{data=@{config=$field.Config}}|Out-Null
  ...
  if($field.RenameFrom-and$attrs.ContainsKey($field.RenameFrom)){
    ...
    if($Apply){
      Request Patch "/objects/deal/attributes/$($current.id.attribute_id)" @{data=@{title=$field.Title;api_slug=$field.Slug}}|Out-Null
  ...
  if($Apply){
    Request Post "/objects/deal/attributes" @{data=@{
...
if($Apply){$attrs=Map @((Request Get "/objects/deal/attributes" $null).data)}
foreach($field in @($fields|Where-Object{$_.Type-eq"select"-and$_.PSObject.Properties.Name-contains"FixedOptions"-and$_.FixedOptions})){
  ...
  $existingTitles=@{}
  foreach($o in @((Request Get "/objects/deal/attributes/$($field.Slug)/options" $null).data|Where-Object{-not$_.is_archived})){$existingTitles[[string]$o.title.Trim().ToLowerInvariant()]=$true}
  foreach($title in $field.FixedOptions){
    $key=$title.Trim().ToLowerInvariant()
    if($existingTitles.ContainsKey($key)){continue}
    $actions.Add("create_option:$($field.Slug):$title")
    if($Apply){
      Request Post "/objects/deal/attributes/$($field.Slug)/options" @{data=@{title=$title}}|Out-Null
      Write-Host "CREATED OPTION: $($field.Slug) -> $title"
    }else{Write-Host "DRY RUN: would create $($field.Slug) option '$title'."}
  }
}
# The pipeline Status field on this custom object was created manually --
# its slug isn't guaranteed to be "stage" the way SOURCE's native Deals
...
foreach($title in $sourceStageTitles){
  $key=$title.Trim().ToLowerInvariant()
  if($existingStageTitles.ContainsKey($key)){continue}
  $actions.Add("create_stage:$($stageSlug):$title")
  if($Apply){
    Request Post "/objects/deal/attributes/$stageSlug/statuses" @{data=@{title=$title}}|Out-Null
    Write-Host "CREATED STAGE: $title"
  }else{Write-Host "DRY RUN: would create deal stage '$title'."}
}
# teaser_status/nda_status/assigned_advisor mirror SOURCE's native Deals
# select fields of the same slug. Confirmed live (2026-08-27): objects.ps1's
# per-record apply loop already auto-creates whichever of these options a
# record actually uses, but that only ever produces the subset SOURCE data
# currently exercises -- SOURCE's own configured dropdowns have extra
# choices no live record happens to use right now ("Advisor Review"/
# "In Draft" on teaser_status, "Partially Signed" on nda_status, and
# assigned_advisor's full "Hugo"/"Jules"/"Ramzy" roster vs. whichever
# advisor(s) are actually assigned today), which DEV's copy of this object
# already carries in full. Seeding the complete SOURCE option list here up
# front (not just what's in use) keeps this object's own dropdown a true
# match for SOURCE's, not merely for today's data.
foreach($mirroredSlug in @("teaser_status","nda_status","assigned_advisor")){
  $sourceOptionTitles=@((Request Get "/objects/deals/attributes/$mirroredSlug/options" $null).data|Where-Object{-not$_.is_archived}|ForEach-Object{[string]$_.title})
  $existingOptionTitles=@{}
  foreach($o in @((Request Get "/objects/deal/attributes/$mirroredSlug/options" $null).data|Where-Object{-not$_.is_archived})){$existingOptionTitles[[string]$o.title.Trim().ToLowerInvariant()]=$true}
  foreach($title in $sourceOptionTitles){
    $key=$title.Trim().ToLowerInvariant()
    if($existingOptionTitles.ContainsKey($key)){continue}
    $actions.Add("create_option:$($mirroredSlug):$title")
    if($Apply){
      Request Post "/objects/deal/attributes/$mirroredSlug/options" @{data=@{title=$title}}|Out-Null
      Write-Host "CREATED OPTION: $mirroredSlug -> $title"
    }else{Write-Host "DRY RUN: would create $mirroredSlug option '$title'."}
  }
}
...
$args=@{DevApiKey=$DevApiKey};if($Entity-ne"deal"){$args.SourceApiKey=$SourceApiKey};if($FailOnDrift-and$Entity-in@("deal","buyer_role","seller_role")){$args.FailOnDrift=$true};if($Apply){$args.Apply=$true}
switch($Entity){"organizations"{Invoke-OrganizationSchema @args};"person"{Invoke-PersonSchema @args};"deal"{Invoke-DealSchema @args};"buyer_role"{Invoke-BuyerRoleSchema @args};"seller_role"{Invoke-SellerRoleSchema @args}}
